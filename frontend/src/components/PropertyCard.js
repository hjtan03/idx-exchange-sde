...
function PropertyCard({ property }) {
  ...
  return (
    <Link to={`/property/${property.L_ListingID}`} className="property-card">
      ...
      <p className="property-card-price">${property.L_SystemPrice?.toLocaleString()}</p>
      <p className="property-card-address">{property.L_Address}</p>
      <p className="property-card-city">{property.L_City}, {property.L_State}</p>
      <p className="property-card-stats">{property.L_Keyword2} beds • {property.LM_Dec_3} baths • {property.LM_Int2_3} sqft</p>
    </Link>
  );
}
